...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < num; i++) // add events to initial queue
			...
			scanf("%lf %f %f %f",&id,&first, &second, &third);
			temptree=makeBranch(id,first,second,third);
			insert(&startPtr,temptree);
			}

		printf("\n Student ID:                  First Score:           Second Score:                Third Score:          Mean: \n");
		print_tree(startPtr); //print the tree out in order after inserting everything
}
...
void print_tree(TreePtr sPtr)
	{
		...
		if (sPtr!=NULL)
		{
			...
			printf("\n Student ID:%lf  First Score:%f Second Score:%f Third Score:%f Mean:%lf \n", sPtr->id,sPtr->first,sPtr->second,sPtr->third,sPtr->mean);
